...
SELECT TOP 6000 GRTZON_DESCRP, USR_FCTTVH_DESCRIP, VTTVND_DESCRP, FCRMVH_USERID, FCRMVH_DEPSAL, FCRMVH_DEPOSI, USR_UBFSEC_DESCRP, USR_FCRMVH_NROAPP, USR_FCRMVH_FCHCIR, USR_FCRMVH_HORCIR, * FROM [dbo].[USR_FCRMVH] 
WHERE FCRMVH_NROGUI = 'T001-26134'
--WHERE USR_FCRMVH_DOCVTA is not NULL
ORDER BY FCRMVH_FECALT desc

-- Salidas de Guias de Remisión
SELECT TOP 2000 * FROM [dbo].[USR_SALGUI]
WHERE SALGUI_NROFOR = '24873'
ORDER BY SALGUI_FECALT desc

-- Anulaciones de Guias de Remisión
SELECT * FROM [dbo].[USR_ANUGUI]
WHERE ANUGUI_TIPREG = 'G'
and ANUGUI_NROGUI = 'T001-51148' --and ANUGUI_NROFOR = 76
...
SELECT FECHA_CIRUGIA, * FROM [dbo].GUIAS_REMISION 
WHERE NUMERO_GUIA = 'T002-5384'  --37725
--LEFT JOIN [USR_ANUGUI] ON ANUGUI_NROGUI = NUMERO_GUIA and ANUGUI_CODEMP = EMPRESA_ID and ANUGUI_TIPREG = 'G'
--WHERE  NUMERO_REFERENCIA = '68369'
ORDER BY id desc

--UPDATE GUIAS_REMISION
--	SET CODIGO_REFERENCIA = i.USR_FCRMVH_CODREF, 
--		NUMERO_REFERENCIA = i.USR_FCRMVH_NROREF, 
--		EMPRESA_REFERENCIA = i.USR_FCRMVH_EMPREF
--FROM [USR_FCRMVH] as i
--WHERE i.FCRMVH_NROGUI = NUMERO_GUIA and i.FCRMVH_CODEMP = EMPRESA_ID --and CODIGO_TRANSFERENCIA is NULL--and id < 57000 and ANUGUI_TIPREG = 'G' 
...
WHERE STATUS_PICKING = 'Picking Retirado' 
	and FECHA_ALMACEN < '2026-06-13 00:00:00.000'
	and USUARIO = 'Jairo Castillo'
ORDER BY id desc
--CREATE INDEX HP_nroGuia ON [dbo].[HOJA_PICKING] (NUMERO_GUIA);
--UPDATE HOJA_PICKING
--	SET USUARIO = 'Jairo Castillo'
--	--SET USUARIO = 'Creado por Servidor'
--	, STATUS_PICKING = 'Picking Retirado'
--	--CONTINGENCIA = 1
--WHERE STATUS_PICKING = 'Picking Terminado' 
--	and FECHA_ALMACEN < '2026-06-14 00:00:00.000'
...
-- Consultas a Descripcion de Guias
SELECT * FROM [dbo].[GR_DESCRIPCION]
WHERE PRODUCTO = 'A4-S0003'  --57636
--GUIA_ID = 57636
ORDER BY id desc

SELECT * FROM [dbo].[USR_STRMVI]
WHERE STRMVI_ARTCOD = 'VMCH-D'
	and STRMVI_SUCURS = 'T003'
	and STRMVI_NROFOR = '51016'

--INSERT INTO [dbo].[GR_DESCRIPCION] (NUMERO_GUIA, PRODUCTO, DESCRIPCION_PRODUCTO, PROVEEDOR, SECTOR, CANTIDAD, LOTE, VENCIMIENTO_LOTE, EMPRESA_ID)
--SELECT g.NUMERO_GUIA, g.PRODUCTO, g.DESCRIPCION_PRODUCTO, g.PROVEEDOR, g.SECTOR, g.CANTIDAD, g.LOTE, g.VENCIMIENTO_LOTE, g.EMPRESA_ID FROM GR_DESCRIPCION as g
--WHERE g.id = 1164211

--Consultas a GR_Busqueda
SELECT * FROM [dbo].GR_BUSQUEDA
WHERE NUMERO_GUIA = 'T001-60233'
ORDER BY id desc

--UPDATE GR_BUSQUEDA
--SET NRO_PROCESO = i.USR_FCRMVH_NROPR
--FROM [USR_FCRMVH] as i
--WHERE i.FCRMVH_NROGUI = NUMERO_GUIA and i.FCRMVH_CODEMP = EMPRESA_ID

--DELETE FROM [GR_DESCRIPCION] WHERE GUIA_ID = 57636
--UPDATE GUIAS_REMISION
...
--UPDATE GUIAS_REMISION
--	SET OBSERVACION = 'ID# 23976 // PED# 92406 // '
--WHERE id = 89850
...
--ALTER TABLE [dbo].[USR_MEDICO]
--ADD RV ROWVERSION;

---- 1. Índice principal para optimizar el Job de abandono de sesión
--CREATE NONCLUSTERED INDEX IX_HOJA_PICKING_AbandonedSession
--ON [dbo].[HOJA_PICKING] (LAST_HEARTBEAT_RECEIVED)
--INCLUDE (APP_SESSION_ACTIVE, ALMACEN, FECHA_ALMACEN, FIRMA_ALMACEN, USUARIO)
--WHERE STATUS_PICKING = 'Picking En Proceso';
--GO

---- 2. Índice complementario para las búsquedas rápidas que hace tu API en Django
--CREATE NONCLUSTERED INDEX IX_HOJA_PICKING_Status_Almacen
--ON [dbo].[HOJA_PICKING] (STATUS_PICKING)
--INCLUDE (ALMACEN, FECHA_ALMACEN);
--GO

ALTER TABLE USR_IDPAPP 
ADD USUARIO VARCHAR(60) NULL;