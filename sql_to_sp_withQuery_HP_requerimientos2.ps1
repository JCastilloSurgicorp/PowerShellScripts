...
function Main() {
    ...
    $sqlQuery = "SELECT TOP 5000 $sqlcollumns FROM SURGICORP_POWERAPPS.dbo.$sqlTable WHERE OC_CLIENTE='' ORDER BY ID_APP DESC"
    $sqlPrimaryKey = "ID"
    # Iniciando Conexión
    ...
    if($sqlSource.Count -eq 0 || $null -eq $sqlSource) {
        ...
        $spQuery = "<View Scope='RecursiveAll'>  
                        <Query>
                            <OrderBy>
                                <FieldRef Name='ID_APP' Ascending='False' />
                            </OrderBy>
                        </Query>
        ...
        # Get List Items 
        $start1 = Get-Date
        $spDestination =  Get-PnPListItem -List $spListName -PageSize 5000 -Query $spQuery
        $end1 = Get-Date
