...
function Main() {
    ...
    $sqlQuery = "SELECT * FROM (SELECT TOP 800 $sqlcollumns FROM SURGICORP_POWERAPPS.dbo.$sqlTable ORDER BY id DESC) AS G ORDER BY G.id"  #FECHA_GUIA >= DATEADD(DAY,-1,CURRENT_TIMESTAMP)"
    $sqlPrimaryKey = "id"
    # Iniciando Conexión
