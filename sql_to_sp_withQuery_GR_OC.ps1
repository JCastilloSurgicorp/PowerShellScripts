...
function Main() {
    ...
    $sqlPass = "*Surgi007"
    # Datos del QUERY
    $sqlTable = "GUIAS_REMISION_OC"
    $sqlcollumns = "id, NUMERO_GUIA, ATENCION, REPRESENTANTE, MOTIVO_TRASLADO, EMPRESA_ID, NOMBRE_CLIENTE, DIRECCION_ENTREGA, FECHA_GUIA, ESTADO, ID_APP, OBSERVACION, OC_CLIENTE, NRO_PROCESO"
    $sqlQuery = "SELECT * FROM (SELECT TOP 100 $sqlcollumns FROM SURGICORP_POWERAPPS.dbo.$sqlTable ORDER BY id DESC) AS G ORDER BY G.id"  #FECHA_GUIA >= DATEADD(DAY,-1,CURRENT_TIMESTAMP)"
    $sqlPrimaryKey = "id"
    # Iniciando Conexión
    ...
    if($sqlSource.Count -eq 0 || $null -eq $sqlSource) {
        ...
        $spFields = "ID","Title","ATENCION","REPRESENTANTE","MOTIVO_TRASLADO","EMPRESA_ID","NOMBRE_CLIENTE","DIRECCION_ENTREGA","ESTADO","ID_APP","OBSERVACIONES","FECHA_GUIA"
        $spQuery = "<View>  
                        <Query>
                            <Where>
                                <Gt>
        ...
        foreach ($row in $sqlSourceHash) {
            ...
            $row[$spFields[5]] = [int]$row["EMPRESA_ID"]
            # Agrega y reemplaza columna ID_APP por entero antes de comparar
            $row[$spFields[9]] = [int]$row["ID_APP"]
